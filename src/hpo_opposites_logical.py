...
def load_opposites_from_tsv():
    ...
    """
    pato_opposites_path = os.path.join(OUTPUT_DIR, "pato_opposites.tsv")
...
def main():
    ...
    # 4) Write to hpo_opposites_logical.tsv with renamed columns and formatted ids
    out_tsv = os.path.join(OUTPUT_DIR, "hpo_opposites_logical.tsv")
    rows = []
    ...
    rows = sorted(rows, key=lambda r: (r["hpo_id1"], r["hpo_id2"], r["bearer_iri"], r["quality1_iri"], r["quality2_iri"]))
    df = pd.DataFrame(rows, columns=[
        "hpo_id1", "hpo_term1", "hpo_id2", "hpo_term2",
        "bearer_iri", "quality1_iri", "quality2_iri"
    ])
    df.to_csv(out_tsv, index=False, sep="\t")
    print(f"[INFO] Found {len(df)} opposite phenotype pairs. Wrote to {out_tsv}")
